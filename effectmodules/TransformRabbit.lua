...
local function morph(char)
	...
	for i=1,coinAmount do
		local morph = game.ServerStorage.wabbajack_misc.Coin:Clone()
		morph.CFrame = hrpcf+Vector3.new(math.random(-10,10),0,math.random(-10,10))
		morph.Parent = workspace
		morph.Anchored = false
		table.insert(morphs,morph)
	end
	for i=1,purseAmount do
		local morph = game.ServerStorage.wabbajack_misc.CoinPurse:Clone()
		morph.CFrame = hrpcf+Vector3.new(math.random(-10,10),0,math.random(-10,10))
		morph.Parent = workspace
		morph.Anchored = false
		table.insert(morphs,morph)
